...
function getString
{
  ...
  do
    ...
    echo "#"
    echo "#"
    echo "# T4K Seedbox Installer Script"
    echo "#"
    echo "#"
    echo
...
if [ "$CHROOTJAIL1" = "YES" ]; then
  cd /etc/t4k-seedbox-install
  tar xvfz jailkit-2.15.tar.gz -C /etc/t4k-seedbox-install/source/
...
echo "PATH=$PATH:/etc/t4k-seedbox-install:/sbin" | tee -a /root/.bashrc > /dev/null
echo "export PATH" | tee -a /root/.bashrc > /dev/null

rm -f /etc/t4k-seedbox-install/ports.txt
for i in $(seq 51101 51999)
do
  echo "$i" | tee -a /etc/t4k-seedbox-install/ports.txt > /dev/null
done

rm -f /etc/t4k-seedbox-install/rpc.txt
for i in $(seq 2 1000)
do
...
if [ "$INSTALLFAIL2BAN1" = "YES" ]; then
  ...
  cp /etc/t4k-seedbox-install/etc.fail2ban.jail.conf.template /etc/fail2ban/jail.conf
...
echo "$NEWUSER1" > /etc/t4k-seedbox-install/mainuser.info
echo "$CERTPASS1" > /etc/t4k-seedbox-install/certpass.info

bash /etc/t4k-seedbox-install/createOpenSSLCACertificate

mkdir -p /etc/ssl/private/
openssl req -x509 -nodes -days 365 -newkey rsa:1024 -keyout /etc/ssl/private/vsftpd.pem -out /etc/ssl/private/vsftpd.pem -config /etc/t4k-seedbox-install/ssl/CA/caconfig.cnf
...
if [ "$OSV1" = "12.04" ]; then
  dpkg -i /etc/t4k-seedbox-install/vsftpd_2.3.2-3ubuntu5.1_`uname -m`.deb
fi
...
if [ "$OSV1" = "14.04" ]; then
  ...
  cp /etc/t4k-seedbox-install/etc.apache2.default.template /etc/apache2/sites-available/000-default.conf
  ...
  cp /etc/t4k-seedbox-install/etc.apache2.default.template /etc/apache2/sites-available/default
...
if [ "$RTORRENT1" = "0.9.4" ]; then
  cd /etc/t4k-seedbox-install/source/xmlrpc
  ./configure --prefix=/usr --enable-libxml2-backend --disable-libwww-client --disable-wininet-client --disable-abyss-server --disable-cgi-server
  make -j$(grep -c ^processor /proc/cpuinfo)
  make install
else
  tar xvfz /etc/t4k-seedbox-install/xmlrpc-c-1.16.42.tgz -C /etc/t4k-seedbox-install/source/
  cd /etc/t4k-seedbox-install/source/
...
cp /etc/t4k-seedbox-install/action.php.template /var/www/rutorrent/plugins/diskspace/action.php
...
cp /etc/t4k-seedbox-install/rutorrent.plugins.filemanager.conf.php.template /var/www/rutorrent/plugins/filemanager/conf.php
...
if [ "$INSTALLSABNZBD1" = "YES" ]; then
  bash /etc/t4k-seedbox-install/installSABnzbd
fi

if [ "$INSTALLRAPIDLEECH1" = "YES" ]; then
  bash /etc/t4k-seedbox-install/installRapidleech
fi
...
perl -pi -e "s/#meter-disk-td \{\ background\: url\(\.\/images\/hd.pn\)\ 5px no-repeat\; \}/#meter-disk-td \{\ background\: url\(\.\/images\/hd.png\)\ 5px no-repeat\;display\:none\;\}/g" /var/www/rutorrent/plugins/oblivion/plugins.css
